...
setMethod(f = "performance",
          signature = "Strategy",
          definition = function(object, of=c("portfolio", "assets"), from, until, which, type=c("performance", "logReturns", "returns"), use.backtest, include.costs) {
            of <- match.arg(of)
            type <- match.arg(type)
...
#' VaR(myStrat.MA, from="2015-01-01", until="2015-12-31", use.backtest=T)
#'
#' ## End(Not run)
setMethod(f = "VaR",
          signature = "Strategy",
...
#'
#' # Get backtest ES of MA(200)-Strategy
#' ES(myStrat.MA, from="2015-01-01", until="2015-12-31", use.backtest=T)
#'
#' ## End(Not run)
setMethod(f = "ES",
          signature = "Strategy",
...
setGeneric(name = "MDD",
           def = function(object, of="portfolio", from=NULL, until=NULL, which=NULL, type="relative", include.costs=T, use.backtest=F) {
             standardGeneric("MDD")
           }
)
...
# Returns the Hit Ratio of the strategy
# either on portfolio or single asset basis
setGeneric(name = "hitratio",
           def = function(object, of="portfolio", from=NULL, until=NULL, which=NULL, type="per.signal", include.costs=T, use.backtest=F) {
...
#' hitratio(myStrat.MA, from="2015-01-01", until="2015-12-31", use.backtest=T)
#'
#' ## End(Not run)
...
#' compare(myStrat.MA, myStrat.EWMA, from="2015-01-01", until="2015-12-31", use.backtest=T, scaling.periods=252)
#'
#' ##End(Not run)
setMethod(f = "compare",
