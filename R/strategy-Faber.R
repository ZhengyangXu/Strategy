...
strategy.faber <- function(prices, weights=NULL, indicators=NULL, parameters=list(), printSteps=F) {
  ...
  # OUTPUT
  return( list(strat.vals=list(Faber.vals=strat.vals), signals=signals, prices=prices, logReturns=logReturns, weights=weights, indicators=indicators, parameters=parameters) )
}
