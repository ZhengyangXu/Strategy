...
# STRATEGY
strategy.ma <- function(prices, weights=NULL, indicators=NULL, parameters=list(), printSteps=F) {
  ...
  strat.vals <- Reduce(cbind, lapply(prices, rollmean, k=k, align="right"))
  ...
  # OUTPUT
  return( list(strat.vals=list(MA.vals=strat.vals), signals=signals, prices=prices, logReturns=logReturns, weights=weights, indicators=indicators, parameters=parameters) )
}
...
plot.ma <- function(object, from=NULL, until=NULL, which=NULL, main=NULL) {
  ...
  strat.vals <- getStratVals(object)[["MA.vals"]][index(prices), colnames(prices)]
